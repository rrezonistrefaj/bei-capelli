...
export function parseRichText(blocks: RichTextBlock[] | string | null | undefined): string {
  if (!blocks) return ''
      ...
      if (block.type === 'list' && Array.isArray(block.children)) {
        ...
        return (
          ...
          block.children
            ...
            .map((item: RichTextBlock) => `<li>${parseRichText(item.children as RichTextBlock[])}</li>`)
            .join('') +
          `</${tag}>`
        )
      }
  ...
  // Client-side: return unsanitized HTML (trusted content from Strapi)
  // Sanitization should happen server-side before sending to client
  return html
}
